...
public readonly struct AssetsApi
{
    ...
    public Task<HttpResponseMessage> List(uint pagination = 5_000) => _apiConfiguration.CreateClient()
        .GetAsync($"{Route}?limit={pagination}");

    public Task<HttpResponseMessage> Details(string asset, uint pagination = 20_000) => _apiConfiguration.CreateClient()
